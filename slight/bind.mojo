...
__extension Int(BindIndex):
    fn idx(self, stmt: Statement) raises -> UInt:
        ...
        Raises:
            Error: Never raises for Int indexing.
        """
        # No validation - direct conversion
        return UInt(self)


__extension UInt(BindIndex):
    fn idx(self, stmt: Statement) raises -> UInt:
        """Returns the index directly without validation.

        Args:
            stmt: The statement (unused for UInt indexing).

        Returns:
            The parameter index.

        Raises:
            Error: Never raises for UInt indexing.
        """
...
__extension String(BindIndex):
    fn idx(self, stmt: Statement) raises -> UInt:
        ...
        if not result:
            raise Error("InvalidParameterName: " + self)
        return result.value()
...
__extension StringSlice(BindIndex):
    fn idx(self, stmt: Statement) raises -> UInt:
        ...
        if not result:
            raise Error("InvalidParameterName: " + name)
        return result.value()
